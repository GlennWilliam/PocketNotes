...
export default function NoteDetailPage() {
	...
	const handleSubmitPassword = async () => {
		if (!password.trim()) return message.warning("Enter password first");
		setSubmitting(true);
		...
		try {
			...
			if (res.ok) {
				...
				if (/Invalid|Wrong/i.test(res.error)) {
					setPassError("Wrong password, please try again.");
				}

				const hinted =
					res?.data?.password_hint ||
					note?.password_hint ||
					passwordHint;
	...
	return (
		<>
			<div className="max-w-3xl mx-auto">
				{note && <CardNote note={note} bg={bg} showPrivateIcon />}
			</div>
