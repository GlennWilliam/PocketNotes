...
export const EditProfileModal = ({ open, onClose }) => {
	...
	const onPickImg = (setter) => (e) => {
		...
		if (!/image\/(jpe?g|gif)/i.test(f.type)) {
			message.warning("Image format not supported. Only JPG, JPEG, and GIF are allowed.");
			e.target.value = "";
			return;
		}
		setter(f);
		...
		>
			...
			<div className="mt-4 flex items-center gap-4">
				...
				<div className="flex flex-wrap gap-3">
					<input
						...
						className="hidden"
						onChange={onPickImg(setProfileFile)}
					/>
					<Button
						className="!h-10 !rounded-full !px-4 border-(--secondary-color) !text-(--secondary-color)"
					...
					<input
						...
						accept="image/png,image/jpeg,image/jpg,image/gif"
						className="hidden"
						onChange={onPickImg(setThumbnailFile)}
					/>
					<Button
						className="!h-10 !rounded-full !px-4 border-(--secondary-color) !text-(--secondary-color)"
			...
			>
				<Form.Item
					name="username"
					label="Username"
					rules={[{ required: true, message: "Name must be filled" }]}
				>
					<Input
